...
#'   Defaults to \code{TRUE} and if \code{includeP = TRUE} it must be \code{TRUE}.
#' @return A character string with stars
#' @keywords misc
#' @export
#' @examples
...
#' formatPval(c(.00052456, .000000124, .01035, .030489, .534946), 3, 3, FALSE, FALSE)
#' formatPval(c(.00052456, .000000124, .01035, .030489, .534946), 5)
formatPval <- function(x, d = 3, sd, includeP = TRUE, includeSign = TRUE) {
  if (includeP) {
    includeSign <- TRUE
  }
  if (missing(sd)) {
  ...
  if (!includeP & !includeSign) {
    format.pval(pv = x, digits = d, scientific = sd)
